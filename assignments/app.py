...
def person():
    if request.method == "GET":
        ...
        name = request.form.get("name")
        family = request.form.get("family")
        status, message = PatientController.edit(id, name, family)
        return render_template("person.html", person_list=PatientController.find_all())
...
@app.route("/api")
def api():
    return jsonify(PatientController.find_all()[1])


if __name__ == "__main__":
    app.run(host="192.168.39.100", port=80, debug=True)# todo : view ??? web/tkinter